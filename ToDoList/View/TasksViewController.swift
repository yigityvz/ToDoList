...
class TasksViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return tableView == self.tableView ? tasks.count : completedTasks.count
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let task = tableView == self.tableView ? tasks[indexPath.row] : completedTasks[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if tableView == self.tableView {
            ...
            let alert = UIAlertController(title: "Tamamlandı mı?", message: nil, preferredStyle: .alert)
    ...
    @objc func handleAddButtonTapped() {
        ...
        guard let self = self else { return }
            ...
            if let existingData = UserDefaults.standard.data(forKey: "tasks"),
               let decoded = try? JSONDecoder().decode([Task].self, from: existingData) {
                allTasks = decoded
            }
            ...
            allTasks.append(newTask)
                ...
                if let encoded = try? JSONEncoder().encode(allTasks) {
                    ...
                    if let taskDate = newTask.date, Calendar.current.isDateInToday(taskDate) {
        ...
        tasks = []
            ...
            if let data = UserDefaults.standard.data(forKey: "tasks"),
               let savedTasks = try? JSONDecoder().decode([Task].self, from: data) {
                self.tasks = savedTasks.filter { task in
            ...
            if let completedData = UserDefaults.standard.data(forKey: "completedTasks"),
               let savedCompleted = try? JSONDecoder().decode([Task].self, from: completedData) {
                self.completedTasks = savedCompleted.filter { task in
                    if let date = task.date {
                        return Calendar.current.isDateInToday(date)
                    }
                    return false
    ...
    func updateProgress() {
        let total = tasks.count + completedTasks.count
        let completed = completedTasks.count
        let progress: Float = total > 0 ? Float(completed) / Float(total) : 0.0
